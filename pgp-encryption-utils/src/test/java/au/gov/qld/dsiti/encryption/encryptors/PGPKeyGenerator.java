...
/**
 * This Utility generates a NEW PGP Secret Key Ring, a Public Key Ring , and a Public Encryption Key.
 *
 * Secret Key Ring:
 *  Contains the Private Keys.
 *  Extension will be .asc
 *  Requires a PassPhrase to access
 *  Format will be binary
 *
 * Public Key Ring:
 *  Contains the Public Keys for BOTH Signing and Encryption
 *  Extension will be .pkr
 *  Format will plain text
 *  File header will be: -----BEGIN PGP PUBLIC KEY BLOCK-----
 *
 *
 * Public Encryption Key:
 *  Contains the Public Key that will be used for Encryption
 *  Extension can be .txt
 *  Format will plain text
 *  File header will be: -----BEGIN PGP MESSAGE-----
 */
public class PGPKeyGenerator {

    public static void main(String args[]) throws Exception {
        if (args.length < 4) {
            System.out.println("Requires 4 Arguments: <key_id> <pass_phrase> <public key ring file> <public encryption key> <private key file>");
            System.exit(-1);
        }

        String keyId = args[0];
        String passPhrase = args[1];
        String publicKeyRingFile = args[2];
        String publicEncryptionKeyFile = args[3];
        String privateKeyFile = args[4];

        PGPKeyRingGenerator krgen = generateKeyRingGenerator(keyId, passPhrase.toCharArray());

        // Generate public key ring, dump to file.
        PGPPublicKeyRing pkr = krgen.generatePublicKeyRing();
        ArmoredOutputStream pkrout = new ArmoredOutputStream(new BufferedOutputStream(new FileOutputStream(publicKeyRingFile)));
        pkr.encode(pkrout);
        pkrout.close();

        PGPPublicKey publicKey = PGPUtils.readPublicKeyFromPGPPublicKeyRing(pkr);
        //System.out.println("Got PublicKey: \n" + new String(publicKey.getEncoded()));
        ArmoredOutputStream pubout = new ArmoredOutputStream(new BufferedOutputStream(new FileOutputStream(publicEncryptionKeyFile)));
